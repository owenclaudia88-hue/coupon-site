...
export default function CaptchaRedirectClientWrapper() {
  ...
  // For now we auto-verify (no puzzle), same as your Elgiganten wrapper
  useEffect(() => {
    setVerified(true)
  }, [])
  ...
  useEffect(() => {
    ...
    fetch("/api/get-offer-url", {
      ...
      // IMPORTANT: include store so the API resolves NetOnNet even if no referer
      body: JSON.stringify({ id: offerId, store: "netonnet" }),
    })
      .then((res) => {
